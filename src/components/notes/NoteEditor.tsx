...
interface NoteEditorProps {
  note?: Note;
  onSave: (note: Omit<Note, "id" | "createdAt" | "updatedAt" | "lastEditedAt" | "version">) => void;
  onCancel: () => void;
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (note?.id) {
        ...
        insertAtCursor(`![${image.name}](${image.url})`, "");
      } else {
        // We're creating a new note, need to handle this after note creation
        // For now, you could use a temporary URL
        const tempUrl = URL.createObjectURL(file);
        insertAtCursor(`![${file.name}](${tempUrl})`, "");
      }
    } catch (error) {
          ...
          <TabsContent value="edit">
            <div className="border rounded-md p-1 mb-3">
              <div className="flex flex-wrap items-center gap-1 p-1 border-b">
                ...
                <Button variant="ghost" size="sm" onClick={handleImageClick} title="Image">
